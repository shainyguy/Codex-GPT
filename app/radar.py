...
class Radar:
    def __init__(self, app: Application, db: Database, sources: list[Source], poll_interval: int, orders_cache: dict[str, str]) -> None:
        self.app = app
        self.db = db
        ...
        for source in self.sources:
            ...
            for user_id in users:
                ...
                for order in orders:
                    ...
                    if is_seen:
                        continue
                    self.orders_cache[order.external_id] = f"{order.title}\n{order.description}\n{order.url}"
                    keyboard = InlineKeyboardMarkup(
                        [[InlineKeyboardButton("Сгенерировать отклик", callback_data=f"gen:{order.external_id}")]]
                    )
                    text = (
                        f"🔥 {escape(order.title)}\n"
                        f"Источник: {escape(order.source)}\n"
                        f"Категория: {escape(order.category)}\n"
                        f"{escape(order.description[:400])}\n"
                        f"Ссылка: {order.url}"
                    )
                    try:
                        await self.app.bot.send_message(user_id, text, reply_markup=keyboard)
                    except BadRequest as exc:
                        logger.warning("Telegram rejected message for user %s (order %s): %s", user_id, order.external_id, exc)
