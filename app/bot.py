...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("cat:"):
        ...
        await query.edit_message_text(f"Категории обновлены: {', '.join(sorted(current or CATEGORIES))}")
        return
    ...
    if data.startswith("gen:"):
        external_id = data.split(":", 1)[1]
        order_text = deps.orders_cache.get(external_id, "Описание заказа недоступно.")
